 ...
 * 
 */
package com.atomicDisorder.remolino.utils;
...
public class Configurations {
	...
	private Logger logMain = Logger.getLogger(Configurations.class.getName());
	private ArrayList<ModuleConfiguration> modules;
	...
	public boolean loadAndValidateConfiguration(Properties defaultProperties, String log4jPropertiesPathName,
		...
		logMain.info("*** " + getParameters().getProperty("app.version"));
		logMain.info("* INITIAL PARAMETERS");
		for (Entry<Object, Object> e : getParameters().entrySet()) {
			logMain.info(e.getKey() + " = " + e.getValue());
		}

		if (!getInvalidParameters().isEmpty()) {
			logMain.fatal("* INVALID INITIAL PARAMETERS");
			for (Entry<Object, Object> e : getInvalidParameters().entrySet()) {
				if (((String) e.getKey()).startsWith("app.")) {
					logMain.fatal("Unmodifible parameter: " + e.getKey() + " (" + e.getValue()+ ")");
				} else {
					logMain.fatal("Unknown parameter: " + e.getKey() + " (" + e.getValue()+ ")");
				}

			}
			logMain.fatal("Recomendation: Check your invalid parameters, correct or just delete them");
			return false;
		}
		return true;
	}
	...
	public ArrayList<ModuleConfiguration> getModules() {
		return modules;
	}

	public void setModules(ArrayList<ModuleConfiguration> modules) {
